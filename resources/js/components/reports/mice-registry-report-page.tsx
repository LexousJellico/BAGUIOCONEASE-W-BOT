...
function officialDomestic(record: MiceRecordRow): number {
    const official = numberValue(record.domestic_attendees);

    return official > 0
        ? official
        : numberValue(record.domestic_participants) +
              numberValue(record.local_participants);
}

function officialForeign(record: MiceRecordRow): number {
    const official = numberValue(record.foreign_attendees);

    return official > 0 ? official : numberValue(record.foreign_participants);
}

function officialVisitors(record: MiceRecordRow): number {
...
function participantTotal(record: MiceRecordRow): number {
    return Math.max(
        numberValue(record.total_participants),
        officialDomestic(record) + officialForeign(record),
        numberValue(record.local_participants) +
            numberValue(record.domestic_participants) +
            numberValue(record.foreign_participants),
    );
}
            ...
            {topRows.length > 0 ? (
                <div className="grid gap-3">
                    {topRows.map((row) => (
                        <article
                            key={row.label}
                            className="mice-report-breakdown-row"
                        >
                            <div>
                ...
                <div className="mice-report-empty-mini">
    ...
    return (
        <article className="mice-report-record-card">
            <main className="grid gap-4">
                <div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
                    <div className="min-w-0">
                        <p className="mice-report-kicker">
                            Record #{record.record_no || record.id} ·{' '}
                            {record.year_recorded || 'No year'}
                        </p>

                        <h3 className="mice-report-title">
                            {text(record.event_name, 'Untitled MICE event')}
                        </h3>

                        <p className="mice-report-muted">
                            {text(record.organization_name, 'No organization')}{' '}
                            · {text(record.venue_area, 'No venue')}
                        </p>
                    </div>
                    ...
                    <div className="flex flex-wrap gap-2 xl:justify-end">
                        ...
                        <span className="mice-report-chip">
                            {text(record.event_category, 'Uncategorized')}
                        </span>

                        {record.booking_status ? (
                            <BookingStatusBadge value={record.booking_status} />
                        ) : null}
                        {record.booking_payment_status ? (
                            <BookingStatusBadge
                                value={record.booking_payment_status}
                                compact
                            />
                        ) : null}
                    </div>
                ...
                <div className="mice-report-detail-grid">
                    ...
                    <div>
                        <p>Participants</p>
                        <strong>
                            {participantTotal(record).toLocaleString()}
                        </strong>
                    </div>
                    ...
                    <div>
                        ...
                        <strong>
                            {formatMoney(
                                record.estimated_tourism_receipts || 0,
                            )}
                        </strong>
                    </div>
                </div>
                ...
                <div className="mice-report-detail-grid is-three">
                    ...
                    <div>
                        <p>Visitor Type</p>
                        <strong>
                            Domestic {officialDomestic(record).toLocaleString()}{' '}
                            / Foreign {officialForeign(record).toLocaleString()}{' '}
                            / Visitors{' '}
                            {officialVisitors(record).toLocaleString()}
                        </strong>
                    </div>
                ...
                <div className="mice-report-detail-grid">
                    <div>
                        <p>Covered Month / Hours</p>
                        <strong>
                            {text(record.covered_month)} /{' '}
                            {numberValue(
                    ...
                    <div>
                        <p>Exhibitions</p>
                        <strong>
                            {boolValue(record.has_exhibitions) ? 'Yes' : 'No'} /{' '}
                            {numberValue(
            ...
            <aside className="mice-report-card-actions">
                {record.booking_id ? (
                    <Link
                        href={bookingShowPath(role, record.booking_id)}
                        className="mice-report-primary-action"
                    >
                        Open Booking
...
export function MiceRegistryReportPage() {
    ...
    const [enterpriseGroup, setEnterpriseGroup] = useState(
        filters.enterprise_group || '',
    );
    const [bookingLinked, setBookingLinked] = useState(
        filters.booking_linked || '',
    );
    ...
    return (
        <BookingRolePageShell
            role={role}
            title="MICE Report"
            description="Booking-linked MICE details, participant totals, visitor origin, room nights, tourism receipts, and export-ready reporting data."
            actions={
                <>
                    <Link
                        href={reportPrintPath(role)}
                        className="mice-report-secondary-action"
                    >
                    ...
                    <a
                        href={reportExportPath(role, filterPayload)}
                        className="mice-report-primary-action"
                    >
            ...
            <section className="grid gap-5">
                <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <StatCard
                        ...
                        description="Submitted MICE reports linked to booking records."
                    ...
                    <StatCard
                        ...
                        value={formatMoney(
                            summary.estimated_tourism_receipts || 0,
                        )}
                ...
                <form
                    ...
                    className="mice-report-filter-panel"
                >
                    ...
                    <FilterControl label="Enterprise">
                        <select
                            value={enterpriseGroup}
                            onChange={(event) =>
                                setEnterpriseGroup(event.target.value)
                            }
                        >
                    ...
                    <FilterControl label="Booking Link">
                        <select
                            value={bookingLinked}
                            onChange={(event) =>
                                setBookingLinked(event.target.value)
                            }
                        >
                            <option value="">All</option>
                ...
                <div className="grid gap-4 xl:grid-cols-3">
                    <BreakdownPanel
                        title="By Category"
                        rows={props.category_breakdown || []}
                        icon={PieChart}
                    />
                    <BreakdownPanel
                        title="By Venue"
                        rows={props.venue_breakdown || []}
                        icon={MapPin}
                    />
                    <BreakdownPanel
                        title="By Origin"
                        rows={props.origin_breakdown || []}
                        icon={BarChart3}
                    />
                </div>
                ...
                {rows.length > 0 ? (
                    ...
                    <section className="mice-report-empty">
                        ...
                        <p>
                            Adjust filters or complete a booking-linked MICE
                            report first.
                        </p>
                    </section>
                )}
