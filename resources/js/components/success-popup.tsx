...
export function triggerPencilBookedSuccessPopup(
    ...
    try {
        window.dispatchEvent(
            new CustomEvent<PencilBookedPopupPayload>(EVENT_NAME, {
                detail: payload,
            }),
        );
    } catch {
...
function safeReadFromSession(): PencilBookedPopupPayload | null {
    try {
        ...
        if (
            typeof parsed.requestedAtIso !== 'string' ||
...
export default function PencilBookedSuccessPopup() {
    ...
    const show = useCallback(
        (nextPayload: PencilBookedPopupPayload) => {
            clearTimers();
            setClosing(false);
    ...
    return createPortal(
        ...
        >
            ...
            >
                <div className="pointer-events-none absolute -top-20 -left-20 h-48 w-48 rounded-full bg-emerald-400/20 blur-3xl" />
                <div className="pointer-events-none absolute -right-20 -bottom-24 h-56 w-56 rounded-full bg-amber-300/20 blur-3xl" />
                ...
                <button
                    type="button"
                    onClick={close}
                    className="absolute top-4 right-4 z-10 grid h-10 w-10 place-items-center rounded-full border border-black/10 bg-white/75 text-slate-500 transition hover:bg-white dark:border-white/10 dark:bg-white/8 dark:text-slate-300 dark:hover:bg-white/12"
                ...
                <div className="relative flex flex-col items-center text-center">
                    ...
                    <p className="text-xs font-black tracking-[0.26em] text-emerald-700 uppercase dark:text-emerald-300">
                        Success · Pencil Booked
                    </p>

                    <h2 className="mt-3 max-w-xl text-3xl font-semibold tracking-[-0.055em] text-slate-950 sm:text-4xl dark:text-white">
                        Your booking is pencil booked for 24 hours.
                    </h2>

                    <p className="mt-3 max-w-xl text-sm leading-7 text-slate-600 dark:text-slate-300">
                        The request was received by BCCC EASE. Please complete
                        the payment requirement before the deadline to keep the
                        selected schedule protected.
                    </p>
                    ...
                    <div className="mt-5 flex w-full max-w-xl items-start gap-3 rounded-[1.25rem] border border-amber-200/80 bg-amber-50/90 p-4 text-left text-sm leading-relaxed text-amber-950 dark:border-amber-400/20 dark:bg-amber-400/10 dark:text-amber-100">
                        ...
                        <p>
                            ...
                            . Failure to comply within the required period may
